...
export default class VehiclePhotosScreen extends Component {
  ...
  continue = async () => {
    ...
    if (this.state.images.length !== 4) {
      Alert.alert("Missing Photos", "Please take all four images", [
        {
          text: "Ok",
    ...
    return (
      <View style={styles.RNContainer}>
        ...
        >
          ...
          {!this.state.snapTapped && (
            <TouchableOpacity
              onPress={() => this.setState({ renderCamera: false })}
              style={styles.RNCapture}
            >
              <Ionicons
                style={{ alignSelf: "center" }}
  ...
  renderAddImageBox(index, view, sampleImage) {
    return (
      <TouchableOpacity
        ...
        onPress={() => this.setState({ renderCamera: true, imageIndex: index })}
      >
        {this.state.images[index] !== undefined ? (
          ...
          <View>
            ...
            <Image source={sampleImage} style={styles.itemImage} />
  ...
  renderContents() {
    return (
      <View style={styles.container}>
        <ScrollView>
          <Text>
            Take Photos of your car as illustrated below, capture both sides of
            the vehicle, the front view and the back view. Make sure the plate
            number is clearly visible. Tap on the sample photo to open your
            camera
          </Text>
          <View style={styles.itemImages}>
            {this.renderAddImageBox(
              0,
              "Left side view",
              R.images.car_left_side
            )}

            {this.renderAddImageBox(
              1,
              "Right side view",
              R.images.car_right_side
            )}
          </View>
