...
import styles from './style'
 ...
 export default class InsuredDamagePhotosScreen extends Component {
  ...
  continue = async () => {
      ...
      const { insuredDamageImages } = this.state;

      const params = { ...this.params, insuredDamageImages };

      this.props.navigation.navigate('ThirdPartyDamagePhotos', {params});
  ...
  takePicture = async () => {
    ...
    if (this.camera) {
      ...
      console.log(data);
      const { insuredDamageImages } = this.state;
      insuredDamageImages.push(data);
      await this.setState({insuredDamageImages, renderCamera: false, capturing: false});
  ...
  removeImage(index) {
    const { insuredDamageImages } = this.state;
    insuredDamageImages.splice(index, 1);
    
    this.setState({imaginsuredDamageImageses});
  }
  ...
  renderImages() {
    ...
    if (insuredDamageImages.length) {

      return(
        <View>
          {insuredDamageImages.map((image,index) => (
    ...
    return (
              <ScrollView>
                ...
                <Text style={styles.cardText}>
                  Upload photos of damages on YOUR VEHICLE if you have any. NOTE claims with photos will be processed faster.
